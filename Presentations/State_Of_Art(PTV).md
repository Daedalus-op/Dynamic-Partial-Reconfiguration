...
A High-Performance and Partially Reconfigurable FPGA Virtualization Framework for Multitenant Networks

<!--
The paper identifies key challenges in this domain, including efficient hardware resource management, maximizing utilization, and virtualization for cloud computing
-->

#### Use of Partial Reconfiguration (PR):

- PR is employed to enable dynamic allocation, isolation, and management of FPGA resources among multiple tenants sharing the hardware.
- The back bone od this architecture is Dynamic Function Exchange, AMD's approach to PR
...
![bg right:40% w:90%](./assets/RandOhm.jpg)

## Security

### **RandOhm: Mitigating Impedance Side-Channel Attacks**

- Attack models: Active sensing & backscattering side-channel attacks (SCA)
- Defense: Moving Target Defense (MTD)
  - Randomize impedance by randomizing physical circuit structure, Achieved using Partial Reconfiguration (PR)
- User specifies target modules
- Tool generates- Multiple randomized partial bitstreams along with original bitstream
- Runtime Phase
  - OS generates a random number trigger
  - PR Generator Unit + ByteMan (open-source bitstream manipulation tool)
    - Selects & randomizes module location
    - Programs FPGA dynamically when triggered
...
- FPGA Compilation is a Major Bottleneck. Vendor tools perform a monolithic placement and routing process with limited parallelism, failing to fully exploit modern multi-core CPUs or cloud computing power.
